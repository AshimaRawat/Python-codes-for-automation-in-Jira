...
class JIRA(object):
    ...
    def get_project_by_key(self, key):
        projects = self.get_projects()
        project = list((filter(lambda d: d['key'] == key, projects)))
        return project
    ...
    def create_issue(self, title, subject, project, issue_type=None, component=None, assigne=None, Priority=None):
        jdata = {}
        jdata['fields'] = {}
        fields = jdata['fields']
        ...
        if component:
            c = list(filter(lambda d: d['name'] == component,
...
if __name__ == "__main__":
    ...
    for row in reader:
        jira.create_issue(row['title'],row['subject'],project=row['project'],issue_type=row['type'],component=row['component'],assigne=row['assignee'],Priority=row['Priority'],)
